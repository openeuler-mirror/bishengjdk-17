...
bool ProfileCacheClassChain::compile_method(ProfileCacheMethodHold* mh) {
  ...
  if (m() == nullptr || m->compiled_by_jprofilecache()) {
    return false;
  }

  log_debug(jprofilecache)("[JitProfileCache] METHOD_CHECK: %s before compilation",
...
void ProfileCacheClassChain::unload_class() {
  ...
  for (int i = 0; i < length(); i++) {
    ...
    for (ProfileCacheMethodHold* holder = entry->method_holder(); holder != nullptr;
      ...
      if (holder->is_method_deopted() || holder->resolved_method() == nullptr) {
        continue;
      }
      if (!holder->is_alive()) {
